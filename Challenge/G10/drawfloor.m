function [GeIm] = drawfloor(EstimatedVertex,rough_coefficient,OrIm)
...
%% floor (z:1~3, x:1~2)
    ...
    worldVertex(2,:) = EstimatedVertex(4,2); % Vertices of the mesh on the same plane (Y = 0);

    for k = EstimatedVertex(5,2):rough_coefficient:EstimatedVertex(5,4) % from z1 to z3

        worldVertex(3,[1,4]) = k; % Z of 1. and 4. point of the mesh equals to k 
        worldVertex(3,[2,3]) = k + rough_coefficient; % z coordinate of 2. and 3. should be k + Coeff.

        for l = EstimatedVertex(3,2):rough_coefficient:EstimatedVertex(3,3) % from x1 to x2

            worldVertex(1,[1,2]) = l; % x of 1. and 2. is l.
            worldVertex(1,[3,4]) = l+rough_coefficient; % x of 3. and 4. is l + Coeff. 
