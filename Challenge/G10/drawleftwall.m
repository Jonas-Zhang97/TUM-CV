function [GeIm] = drawleftwall(EstimatedVertex,rough_coefficient,OrIm)
...
screenPixelVertex = zeros(2,4);
    ...
    % vertices 1,7,11,5
    worldVertex(1,:) = EstimatedVertex(3,2); % Vertices of the mesh on the same plane (X = 0);

    for k = EstimatedVertex(4,2):rough_coefficient:EstimatedVertex(4,8) % from y1 to y7

        worldVertex(2,[1,4]) = k; % y of 3. and 4. point of the mesh equals to k (1,5)
        worldVertex(2,[2,3]) = k + rough_coefficient; % y coordinate of 1. and 2. should be k + Coeff. (7,11)

        for l = EstimatedVertex(5,2):rough_coefficient:EstimatedVertex(5,6) % from z1 to z5

            worldVertex(3,[1,2]) = l; % z of 1. and 4. is l. (1,7)
            worldVertex(3,[3,4]) = l+rough_coefficient; % z of 2. and 3. is l + Coeff.  (11,5)
